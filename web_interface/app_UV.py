...
def parse_filename_and_acquire_data(filename, method):
    ...
    if method == 'normal':
        ...
        Arduinos.serial_controllers['waterscope'].serial_write('UV_on', parser='waterscope')
        time.sleep(2)
        Camera.take_image(resolution='high_res', filename=filename)
        time.sleep(2)
        Arduinos.serial_controllers['waterscope'].serial_write('UV_off', parser='waterscope')
