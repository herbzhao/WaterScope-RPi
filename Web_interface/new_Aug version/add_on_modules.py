...
class time_lapse_module:
    def __init__(self):
        self.camera = picamera.PiCamera()
        time.sleep(2)
        time_stamp = str(datetime.datetime.now())
        self.image_filepath = '/home/pi/images/images-{}/'.format(time_stamp)
        if not os.path.isdir(self.image_filepath):
            os.mkdir(self.image_filepath)

    def save_images(self):
        i = 0 
        while True:
            print('taking image # {}'.format(i))
            self.camera.capture(self.image_filepath +'IMG_{}'.format(i) + '.jpeg', format = 'jpeg', bayer = True)
            i += 1
            time.sleep(3)
