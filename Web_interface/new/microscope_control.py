...
class camera_control():
    ...
    def update_picamera_parameters(self):
        '''
        update the picamera parameters based on info written in  microscope_config.txt, which is updated by web interface or predefined.
        '''
        config = initialise_config()
        config.read_config_file()
        self.camera.resolution = (config.resolution_x, config.resolution_y)
        ...
        self.camera.annotate_text_size = int(config.resolution_x*0.03) # reasonable ratio for any resolution
    ...
    def camera_library(self, argv, *value):
        ...
        elif argv == 'start_taking_streaming_images':
            # save images at temperory folder
            self.streaming_images = self.camera.capture_continuous('tmp/images/stream_image.jpg', 'jpeg', use_video_port=True)
        elif argv == 'stop_taking_streaming_images':
            # stop the camera.capture_continuous
            self.streaming_images = None
