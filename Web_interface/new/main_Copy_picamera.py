...
# allows to test run the app on computer without importing pi specific library
config_mode = True
...
#subprocess.Popen(['mjpg_streamer', '-o', 'output_http.so -w ./www', '-i', 'input_raspicam.so'])
#mjpg_streamer -o "output_http.so -w ./www" -i "input_raspicam.so"

#sudo raspistill -tl 50 -t 100000 -o /tmp/mjpg/test.jpg -w 640 -h 480 -q 5 
#mjpg_streamer -o "output_http.so -w ./www" -i "input_file.so -f /tmp/mjpg/ -n test.jpg"
...
# Picamera streaming using mjpeg_streamer
class MJpegStream_Picamera():
    def __init__(self):
        if config_mode is False:
            self.camera = 
        self.proc = None
        # mjpeg_streamer with file in tmp folder
        self.mjpeg_streamer_parameters =  ['mjpg_streamer', '-o', 'output_http.so -w ./www', '-i', 'input_file', '/tmp/']
    ...
    def start_stream(self):
        ...
        Start mjpg-streamer, wait until it has warmed up, and return success/failure
        """
        # start picamera and update parameters
        self.update_picamera_parameters()
        # start the camera and use the parameters in config.txt
        self.camera_library('start_taking_streaming_images')
        # if service already exist, stop the MJPEG-streamer
        if not self.proc:
            self.stop()
        # start the MJPEG-streamer from commandline
        # subprocess.Popen(['arg1','arg2','arg3']) --> command line: arg1 arg2 arg3 (e.g. subprocess.Popen(['ping','10.0.0.1'])) 
        self.proc = subprocess.Popen(self.mjpeg_streamer_parameters,
                                     stderr=subprocess.PIPE, universal_newlines=True)
