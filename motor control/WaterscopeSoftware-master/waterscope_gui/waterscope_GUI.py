...
class create_control_elements(object):
    ...
    def create_exit_button(self):
        ...
        def exit_GUI(instance):
            ...
            if debug_mode is False:
                self.mc.camera_library('stop_preview')
            raise SystemExit(0)
    ...
    def create_map_controller(self):
        ...
        def map_control_feedback(instance, value):
            ...
            if debug_mode is True: 
                ...
                if map_controller.center[0] - Window.center[0] > self.drag_x_sensitive:
                    self.mc.stage_library('move_x', '-')
                elif map_controller.center[0] - Window.center[0] < -1* self.drag_x_sensitive:
                    self.mc.stage_library('move_x', '+')
                elif map_controller.center[1] - Window.center[1] > self.drag_y_sensitive:
                    self.mc.stage_library('move_y', '-')
                elif map_controller.center[1] - Window.center[1] < -1*self.drag_y_sensitive:
                    self.mc.stage_library('move_y', '+')
                elif map_controller.scale < default_scale*0.9:
                    self.mc.camera_library('zoom_out')
                elif map_controller.scale > default_scale*1.1:
                    self.mc.camera_library('zoom_in')
    ...
    def create_preview_buttons(self):
        ...
        def preview_control(instance):
            ...
            if debug_mode is True:
                ...
                if instance.text == 'preview':
                    self.mc.camera_library('start_preview')
                else:
                    self.mc.camera_library('stop_preview')
    ...
    def create_settings_button(self):
        ...
        def preview_window_resize(argv, instance):
            ...
            if debug_mode is False:
                if argv == 'fullscreen':
                    self.mc.camera_library('stop_preview')
                    self.mc.camera_library('fullscreen_preview')
                if argv == 'resized':
                    self.mc.camera_library('stop_preview')
                    reduced_size_window = (
                        int(Window.width*0.2), int(Window.height*0.25),
                        int(Window.width*0.6), int(Window.width*0.6/1.33))
                    self.mc.camera_library('reduced_size_preview', reduced_size_window)
        ...
        def switch_settings_popup_content(instance):
            ...
            if instance == settings_button:
                ...
                if debug_mode is False:
                    self.mc.camera_library('stop_preview') # the preview will block the keyboard
        ...
        def setting_slider_update_label(instance,*value):
            ...
            for slider, label in [  # for i, j in [[1,2],[3,4]]: >>>i = 1,3  j = 2,4
                ...
                if debug_mode is True:
                    ...
                    self.mc.camera_library('set_brightness', brightness_slider.value)
                    self.mc.camera_library('set_contrast', contrast_slider.value)
                    self.mc.camera_library('set_white_balance', red_gain_slider.value, blue_gain_slider.value)
    ...
    def create_save_image_buttons(self):
        ...
        def save_image(instance):
            ...
            if debug_mode is False:
                self.mc.camera_library('save_image', self.folder, self.filepath, self.format)
        ...
        if debug_mode is False:
            # When click the time_lapse_interval_input, stop the preview
            time_lapse_interval_input.bind(focus = partial(self.mc.camera_library, 'stop_preview'))
    ...
    def create_new_sample(self, *value):
        ...
        # if there is no usb stick inserted
            ...
            self.folder = self.default_folder
        self.folder = self.folder + 'sample_{:03d}'.format(config.new_sample_number)
        self.filename = 'sample_{:03d}'.format(config.new_sample_number)
        self.filetype = '.jpg' 
        self.image_number = 1
        ...
        try:
            self.mc.camera_library('change_annotation', 'Sample number {}'.format(config.new_sample_number))
        except:
            pass
...
class WaterScopeApp(App):
    ...
    def build(self):
        main_page = add_main_page_widgets(self.mc)
        return main_page
        
if __name__ == "__main__":
    if debug_mode:
        WaterScopeApp(None).run()
    else:
        from camera_control import camera_control_context
        with camera_control_context() as mc:
            mc.fov=1
            if expert_mode is False:
                mc.camera_library('start_preview')
            WaterScopeApp(mc).run()
